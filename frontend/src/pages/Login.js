...
function Login() {
  ...
  // ── Normalise FastAPI error — could be string or array ─────────────────
  const extractError = (err) => {
    if (!err.response) return 'Cannot reach server. Check your connection.';
    const detail = err.response.data?.detail;
    if (!detail) return `Login failed (${err.response.status}).`;
    if (typeof detail === 'string') return detail;
    if (Array.isArray(detail)) {
      // FastAPI validation error — pick first message
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.email.includes('@')) {
      setError('Please enter a valid email address (e.g. ansh@gmail.com).');
      return;
    }
              ...
              {formData.password && (
                ...
                >
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"
                    ...
                    {showPassword && <line x1="1" y1="1" x2="23" y2="23" strokeWidth="2" strokeLinecap="round" />}
                  </svg>
                </button>
              )}
            </div>
        ...
        <div className="info-content">
          ...
          <div className="features-list">
            ...
            <div className="feature-item">
              ...
              <span>Live MQTT data pipeline</span>
            </div>
            <div className="feature-item">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                <polyline points="20 6 9 17 4 12" strokeLinecap="round" strokeLinejoin="round" />
